...
Run the built binary with `./cli` or `./cli-dev`. Set `LOCAL_BASE_URL` (default: `http://127.0.0.1:8000/v1`) and optionally `LOCAL_API_KEY` in the environment, or use `/login` to save a local API key.
...
- **LLM query pipeline**: src/QueryEngine.ts coordinates message flow, tool use, and model invocation (routes to local vLLM via src/services/api/openai-local.ts).
- **Core subsystems**:
